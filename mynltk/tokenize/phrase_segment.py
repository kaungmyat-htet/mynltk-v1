...
def connect (words, bond, include_seperator=True):
    N = len (words)
    n = 0
    sentence = []
    while (n < N):
        ...
        if flag == 0:
            ...
            if (include_seperator):
                ...
                sentence.append (words[n] + words[n+1])
            n += 2
    return sentence


def collocate (words, phrases, include_seperator=True):
    N = len(words)
    bond = []
    for n in range(N-1):
    ...
    while True:
        ...
        if n < N-2:
            bond[n+1] = 0
    # join words
    return connect (words, bond, include_seperator)
...
def segment(words, threshold, minfreq, uni_dict_bin, bi_dict_bin, include_seperator):
    unigram = read_dict(uni_dict_bin)
    bigram = read_dict(bi_dict_bin)
    phrases = compute_phrase (unigram, bigram, threshold, minfreq)
    result = []
    if len(words) > 0:
        result = collocate(words, phrases, include_seperator)
